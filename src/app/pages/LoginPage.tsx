...
export function LoginPage({ onLogin }: LoginPageProps) {
  ...
  const [customSideImage, setCustomSideImage] = useState('');
  ...
  const handleSideImageUpload = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.startsWith('image/')) {
      setAuthError('Please select a valid image file for the side panel');
      return;
    }

    const reader = new FileReader();
    reader.onload = () => {
      setCustomSideImage(String(reader.result || ''));
      setAuthError('');
    };
